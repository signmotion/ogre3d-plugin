...
std::shared_ptr< Doxel3D >  Doxel3D::valueOf(
    ...
    const size_t halfN = hmImage.first.get<0>() / 2;
    const size_t halfM = hmImage.first.get<1>() / 2;
    ...
    */
    boost::array< std::shared_ptr< float >,  8 >  octant = {};
    ...
    // *обобщённые* данные о 3D-карте, данные, которые можно поместить в CellCD.
    for (size_t j = 0; j < M; ++j) {
        for (size_t i = 0; i < N; ++i) {
            ...
            */
            ...
            // плоскость XY, можем выделить 2 группы горизонтальных квадрантов.
            ...
            if ( notLastDoxel ) {
                ...
                // отдельному методу.
                const auto initQuadrans = [ &octant, halfNM ] ( int k ) -> int {
                    if ( !octant[k] )  {
                        octant[k] = std::shared_ptr< float >( new float[ halfNM ] );
                        std::fill_n( octant[k].get(),  halfNM,  std::numeric_limits< float >::infinity() );
                    }
                    return k;
                };
                ...
                if (height >= 0.0f) {
                    ...
                    if ( (x >= 0) && (y >= 0) ) {
                        kTop = initQuadrans( 0 );
                        kBottom = initQuadrans( 4 );
                    } else if ( (x >= 0) && (y < 0) ) {
                        kTop = initQuadrans( 1 );
                        kBottom = initQuadrans( 5 );
                    } else if ( (x < 0) && (y < 0) ) {
                        kTop = initQuadrans( 2 );
                        kBottom = initQuadrans( 6 );
                    } else if ( (x < 0) && (y >= 0) ) {
                        kTop = initQuadrans( 3 );
                        kBottom = initQuadrans( 7 );
                    } else {
    ...
    const auto content = std::shared_ptr< CellCD >( new CellCD( 1 ) );
    auto doxel = std::shared_ptr< Doxel3D >( new Doxel3D( doxelSize, content, parent ) );


    // IV Формируем потомков.
    // Размер потомков всегда в 2 раза меньше размера родителя по каждому
    // измерению докселя. Все потомки - одного размера.
    // Создаём потомков:
    ...
    if ( notLastDoxel ) {
        ...
        for (auto itr = octant.cbegin(); itr != octant.cend(); ++itr) {
            // Не каждый октант инициализирован (см. выше)
            if ( *itr != nullptr ) {
                const auto k = std::distance( octant.cbegin(), itr );
                doxel->child()->at( k ) = valueOf(
                    // = parent
                    doxel,
                    std::make_pair( childHMSize,  octant[k] ),
                    childWorldSize
                );
            } else {
                // @test
...
Doxel3D::hmImage_t Doxel3D::loadHMImage(
    ...
    const float kn = 2.0f / (float)(maxHeight - maxDepth + 1);
